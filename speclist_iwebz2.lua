...
local function OnPlayerDeath(event)
    ...
    -- Получаем индекс жертвы
    local victim_index = client.GetPlayerIndexByUserID(event:GetInt("userid"))
    if not victim_index then return end -- Добавлена проверка на nil
    ...
    -- Подсчёт живых врагов с инициализацией переменной
    local enemies_alive = 0 -- Инициализация здесь
    ...
    -- Для отладки (если используется)
    print("Enemies alive: " .. tostring(enemies_alive)) -- tostring() для защиты от nil
end
