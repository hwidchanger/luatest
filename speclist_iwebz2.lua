...
local function OnPlayerDeath(event)
    ...
    local victimIndex = client.GetPlayerIndexByUserID(victimUID)
    local victimEnt = entities.GetPlayerByIndex(victimIndex)
    if not victimEnt then return end
    
    local victimTeam = victimEnt:GetTeamNumber()
    local localPlayer = entities.GetLocalPlayer()
    if not localPlayer then return end
    
    -- Проверка наличия живых игроков в команде жертвы
    local teamAlive = false
    for i = 1, 64 do
        local player = entities.GetPlayerByIndex(i)
        if player and player:GetTeamNumber() == victimTeam and player:IsAlive() then
            teamAlive = true
            break
        end
    end
    
    -- Если в команде жертвы не осталось живых
    if not teamAlive then
        if victimTeam ~= localPlayer:GetTeamNumber() then
            victoryFlag = 1  -- Победа, если умер последний враг
        else
            defeatFlag = 1   -- Поражение, если умер последний союзник
        end
    end
end
...
local function draw_speclist_body(x, y)
    if lastEnemyDeadFlag == 1 then return end
    if spectype == 0 then return end
